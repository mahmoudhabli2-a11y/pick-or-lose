...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "تحدي العقول: لعبة تحدّي العقل والمهارة بالعربية مع تحديات يومية ولوحة متصدرين." },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
      { name: "theme-color", content: "#7c3aed" },
      { name: "apple-mobile-web-app-capable", content: "yes" },
      { name: "apple-mobile-web-app-status-bar-style", content: "black-translucent" },
      { name: "apple-mobile-web-app-title", content: "تحدي العقول" },
    ],
    links: [
    ...
    import("@/integrations/supabase/client").then(({ supabase }) => {
      import("@/lib/auth").then(({ syncProgressFromCloud }) => {
        supabase.auth.getSession().then(({ data }) => {
          if (mounted && data.session?.user) syncProgressFromCloud(data.session.user);
        });
        const { data: sub } = supabase.auth.onAuthStateChange((event, session) => {
          if (event !== "SIGNED_IN" && event !== "SIGNED_OUT" && event !== "USER_UPDATED") return;
          router.invalidate();
          if (event === "SIGNED_IN" && session?.user) {
            syncProgressFromCloud(session.user);
          }
        });
